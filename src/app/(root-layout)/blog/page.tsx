...
import {getCategories, getPostPage} from '@/lib/posts';
...
export default async function Blog() {
    const [result, categories] = await Promise.all([getPostPage(1), getCategories()]);
        ...
        <Container className="py-12 sm:py-16">
            <header className="mb-10">
                ...
                <p className="mt-2 mb-6 text-ink-muted">
                    백엔드, 데이터베이스, 인프라에서 겪은 것들을 기록합니다. 전체 {result.total}건
                </p>
                <SearchForm />
            </header>
            <PostGrid posts={result.posts} categories={categories} />
            <Pagination current={1} totalPages={result.totalPages} hrefFor={blogPageHref} />
        </Container>
    );
}
