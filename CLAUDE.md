...
개인 기술 블로그 + 이력서 사이트 (https://blog.beenslab.com). Next.js 14.2 App Router + Prisma(PostgreSQL), Vercel 배포. 게시글/카테고리는 DB에 저장되고 `/admin`의 마크다운 에디터로 작성한다. `README.md`는 create-next-app 기본 템플릿이라 참고할 내용이 없다(포트도 실제와 다름).
...
```bash
npm run dev            # dev 서버, 포트 7777
npm run build          # 프로덕션 빌드 (DATABASE_URL 필요: 공개 페이지를 빌드 때 prerender)
npm start              # 포트 7777
npm run lint           # next lint (next/core-web-vitals)
npx tsc --noEmit       # 타입체크 (스크립트 없음)
npx prisma generate    # schema.prisma 수정 후 (postinstall에서도 실행됨)
```

- 쉘에 `NODE_ENV=development`가 설정돼 있으면 `next build`가 prerender 단계에서 `Cannot read properties of null (reading 'useContext')`로 실패한다 → `NODE_ENV=production npm run build`.
...
- `npm run deploy`/`predeploy`는 GitHub Pages(static export) 시절 잔재라 동작하지 않는다(`gh-pages` 미설치, `output: 'export'` 주석 처리). 배포는 Vercel.
...
- "redirect to login" 테스트 외에는 실제 DB와 테스트 계정(`beenslab.corp@gmail.com` / `test1234`, bcrypt 해시로 저장)이 필요하다. seed 스크립트는 저장소에 없다(`scripts/`는 gitignore).
- `test-results/.last-run.json`이 git에 추적되고 있어 테스트를 돌리면 working tree가 바뀐다. 커밋에 섞이지 않게 주의.
...
- `src/middleware.ts`가 `/admin/:path*`(`/admin/signin` 제외)에서 JWT를 검증하고 리다이렉트한다. 클라이언트 컴포넌트(Header의 Login/Logout, 에디터)는 `/api/auth/session`으로 로그인 여부와 `userId`를 가져온다.
...
MDX+contentlayer(GitHub Pages) → 외부 백엔드 API + NextAuth OAuth → 현재 구조(2026-01)로 옮겨오면서 남은 것들: 의존성 `firebase`, `axios`, `recoil`, `shiki`, `rehype-pretty-code`, `@types/next-auth`, package.json의 `next-contentlayer` override, `next.config.js`의 `styledComponents` 옵션, `src/data/category.ts`(카테고리는 DB로 이동).
